...
int main(int argc, char* argv[]){
    ...
    average_wait_time=ceil((w/total_num_cpu)*1000)/1000;
    average_turn_time = floor(((average_wait_time+average_cpu_burst+t_cs)*total_num_cpu+t_cs*p)/total_num_cpu*1000)/1000;
    cpu_util = ceil((total_cpu_burst/rt*100)*1000)/1000;
    ...
    average_wait_time= ceil((w-(t_cs/2)*p)/total_num_cpu*1000)/1000;
    average_turn_time = ((average_wait_time+average_cpu_burst+t_cs)*total_num_cpu+t_cs*p)/total_num_cpu;
    cpu_util = ceil((total_cpu_burst/rt*100)*1000)/1000;
...
void RR(std::vector<Process> processes,int t_cs,int t_slice,int&w,int&c,int&p,int&rt){
    ...
    while(n_done<n_pro){
        ...
        //if process arrival time happens
        if(index<n_pro){
            if(processes[index].get_arrival_time()==time){
                //insert to ready queue
                ...
                //reset actual burst time
                a.set_actual_burst(a.get_burst().front());
                a.set_cpu_done(true);
                Q.push(a);
                if(time<1000){
                    std::cout<<"time "<<time<<"ms: Process "<<a.get_id()<<" arrived; added to ready queue [Q ";
                    print_q(Q);
                    std::cout<<"]"<<std::endl;
                }
                index+=1;
        ...
        //if cpu_end_time happened
        if(time == cpu_end_time){
            switch_done = false;
            Process a = cpu[0];
            //remove process from cpu
            cpu.pop_back();
            //if the process get time sliced
            if(a.check_preempt()==true){
                //if there is no process in que
                if(Q.empty()==true){
                    cpu_done_swith_time = -1;
                    if(time<1000){
                    ...
                    if(a.get_cpu_remain_time() <= t_slice){
                        //sets when the cpu is done using
                        cpu_end_time = time + a.get_cpu_remain_time();
                        a.set_preempt(false);
                    }
                ...
                //if there is a process in que preemp
                else{
                    a.set_cpu_done(false);
                    //sets context switch out time;
                    cpu_done_swith_time = time+t_cs/2;
                    if(time<1000){
                        std::cout<<"time "<<time<<"ms: Time slice expired; process "<<a.get_id()<<" preempted with "<<a.get_cpu_remain_time()<<"ms to go [Q ";
                        print_q(Q);
                        std::cout<<"]"<<std::endl;
                    }
                    pt+=1;
            ...
            //if cpu burst ended
                ...
                if(a.get_num_cpu_burst()==0){
                    n_done+=1;
                    std::cout<<"time "<<time<<"ms: Process "<<a.get_id()<<" terminated [Q ";
                    print_q(Q);
                    std::cout<<"]"<<std::endl;
                    //sets context switch out time;
                    cpu_done_swith_time = time+t_cs/2;
                }
                //if that process goes to IO
                else{
                    //sets context switch out time;
                    cpu_done_swith_time = time+t_cs/2;
                    if(time<1000){
                        std::cout<<"time "<<time<<"ms: Process "<<a.get_id()<<" completed a CPU burst; "<<a.get_num_cpu_burst();
                        if(a.get_num_cpu_burst()==1){
                            std::cout<<" burst to go [Q ";
                        }
                        else{
                            std::cout<<" bursts to go [Q ";
                    ...
                    //sets IO end time
                    int io_time = time + (t_cs/2)+a.get_burst().front();
                    a.set_io_time(io_time);
                    //delete IO burst
                    a.pop_burst();
                    if(time<1000){
                        std::cout<<"time "<<time<<"ms: Process "<<a.get_id()<<" switching out of CPU; will block on I/O until time "<<io_time<<"ms [Q ";
                        print_q(Q);
                        std::cout<<"]"<<std::endl;
                    }
                    a.set_cpu_remain_time(a.get_burst().front());
        ...
        if(cpu_empty && Q.size()>0){
            ...
            if(a.get_cpu_remain_time() <= t_slice){
                //sets when the cpu is done using
                cpu_end_time = time + a.get_cpu_remain_time()+(t_cs/2);
                a.set_preempt(false);
            }
            //if cpu burst time is longer than t_slice
            else{
        ...
        if(time == cpu_in_time && cpu_empty == false){
            ...
            if(time<1000){
                std::cout<<"time "<<time<<"ms: Process "<<a.get_id()<<" started using the CPU for ";
                if(a.get_cpu_done()==false){
                    std::cout<<"remaining "<<a.get_cpu_remain_time()<<"ms of "<<a.get_burst().front()<<"ms burst [Q ";
                    print_q(Q);
                    std::cout<<"]"<<std::endl;
                }
                else{
